...
class CopilotClient:
    ...
    mode:
        ``"consumer"`` (default) — use the curl_cffi HTTP/WebSocket driver.
        ``"m365"`` — use Playwright DOM automation against M365 Copilot web UI.
    """
    ...
    def __init__(
        ...
        if mode == "consumer":
            ...
            self._driver = None
            self._browser = BrowserCopilot(proxy=proxy, mode="m365")
    ...
    def stream(
        ...
        **kwargs,
        ...
        if self._mode == "m365":
            self._ensure_ready()
            conv_id = conversation_id or f"m365-{int(time.time())}"
            chunks = self._browser.create_completion(prompt, conversation_id=conv_id, **kwargs)
            return ChatStream(chunks, conv_id)
    ...
    def _ensure_ready(self) -> None:
        """Lazy-start the browser if it hasn't been started yet."""
        if self._browser:
            self._browser.start()  # start() already guards against double-init
